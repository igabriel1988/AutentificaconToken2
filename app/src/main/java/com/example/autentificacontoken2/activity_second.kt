...
class activity_second : AppCompatActivity()
{
    ...
    {
        token = ""
        Toast.makeText(binding.token.context, "Cerrando la session", Toast.LENGTH_SHORT).show()
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if( response.isSuccessful ){
                    try {
                        ...
                        Toast.makeText(binding.token.context, res?.string(), Toast.LENGTH_SHORT).show()
                    }
                    catch (e: IOException)
                    {
                        e.printStackTrace()
                    ...
                    Toast.makeText(binding.token.context, "Token no es correcto", Toast.LENGTH_SHORT ).show()
                }
            }
